    ...
    while (commands[index] != NULL) {
        ...
        if (strcmp(program, "info") == 0) {
            ...
            if (child == 0) {
                ...
                // Parent process
                // Checks for &. If & exists, parent will proceed without waiting. Else, parent will wait.
                ...
                if (no_waiting) {
                    ...
                    if (WIFSTOPPED(status)) {
                        update_child_status(child, 4, status);
                    } else if (WIFSIGNALED(status)) {
                        update_child_status(child, 1, status);
                    } else {
...
void execute_info(char **tokens) {
    ...
    if (argument[1] == NULL || (strcmp(argument[1], "0") != 0 && strcmp(argument[1], "1") != 0 
        && strcmp(argument[1], "2") != 0 && strcmp(argument[1], "3") != 0 && strcmp(argument[1], "4") != 0)) {
    ...
    // Reap processes
    for (int i = 0; i < num_of_processes; i++) {
        if (processes[i].status == 2 || processes[i].status == 3) {
            int code;
            if (waitpid(processes[i].pid, &code, WNOHANG) > 0) {
                processes[i].status = 1;
                processes[i].exitCode = code;
            }
        }
    }
...
void execute_fg(char **tokens) {
    ...
    int pid = atoi(argument[1]);
    for (int i = 0; i < num_of_processes; i++) {
        if (processes[i].pid == pid && processes[i].status == 4) {
            // Resume the process
            printf("[%d] resumed\n", pid);
            kill(pid, SIGCONT);
            ...
            if (WIFSTOPPED(status)) {
                update_child_status(pid, 4, status);
                printf("[%d] stopped\n", pid);
            } else if (WIFSIGNALED(status)) {
                update_child_status(pid, 1, status);
                printf("[%d] interrupt\n", pid);
            } else {
                // int code = WEXITSTATUS(status);
                update_child_status(pid, 1, status);
